...
export function TaskListPage(props: TaskListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_task_assignee_idTouser',
          header: 'User Task Assignee Id Touser',
          accessorKey: 'user_task_assignee_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_task_assignee_idTouser?.id}`}
            >
              {record.user_task_assignee_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_task_waiting_on_idTouser',
          header: 'User Task Waiting On Id Touser',
          accessorKey: 'user_task_waiting_on_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_task_waiting_on_idTouser?.id}`}
            >
              {record.user_task_waiting_on_idTouser?.email}
            </Link>
          ),
        }
      : null,
