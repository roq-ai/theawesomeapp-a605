...
function TaskViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TaskInterface>(
    ...
    () =>
      getTaskById(id, {
        relations: ['user_task_assignee_idTouser', 'user_task_waiting_on_idTouser'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Task Assignee Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_task_assignee_idTouser?.id}`}>
                        {data?.user_task_assignee_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Task Waiting On Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_task_waiting_on_idTouser?.id}`}>
                        {data?.user_task_waiting_on_idTouser?.email}
                      </Link>
                    }
                  />
                )}
